...
We'll talk now about an important family of matrices, called {\em Projection Matrices}.  The are matrices $P$ for which project onto their image.  Let's explain this a bit.  Call $\mathcal P:=\operatorname{Im}(P)$, the image of $P$.  Recall that the image of a matrix is the set of all vectors $|w\rangle$ which equal $P|v\rangle$ for some $|v\rangle$.  Now by Theorem \ref{Vector Decomposition}, we can write any vector as $|v\rangle=|v_\parallel\rangle+|v_\bot\rangle$, where $|v_\parallel\rangle\in\mathcal P$ and $\langle p|v_\bot\rangle=0$ for all $p\in\mathcal P$.  (See also Exercise \ref{Image is subspace}.)  A projection matrix is a matrix for which $P|v\rangle=|v_\parallel\rangle$ for all $|v\rangle$.  We'll argue later that $|v_\parallel\rangle$ is the closest vector in $\mathcal P$ to $|v\rangle$ itself.  It's a nice consequence that what remains, $|v\rangle - |v_\parallel\rangle$ is orthogonal to $\mathcal P$.  The mapping $|v\rangle\to|v_\parallel\rangle$ is sometimes called the "orthogonal projection."  This is what we usually think of as projection.  If you had a sphere in 3D space, and I asked you to project to the xy-plane, you'd probably naturally project it orthogonally.

An immediate consequence of the definition of projection matrices is that it has an {\em Invariant Subspace}.  Namely the entire subspace $\mathcal P$ is fixed by applying $P$ to it.  This subspace also contains {\it all} the vectors that are fixed by $P$.  Because any vector $|v\rangle not in $\mathcal P$ gets decomposed as $|v\rangle=|v_\parallel\rangle+|v_\bot\rangle$, where $|v_bot\rangle\neq0$ (for otherwise $|v\rangle$ would be in $\mathcal P$.

Another consequence is that after you've projected onto $\mathcal P$, there's nothing else to do.  That is $P\left(P|v\rangle)=P|v\rangle$ for all $|v\rangle$.  This is immediate because $P|v\rangle\in\mathcal P$.  This can be written as $P^2=P$.  This is called {\em idempotence}.  Some books will give idempotence as a definition for projection matrices.  We'll see later that idempotence implies our above definition.

{\bfseries\scshape\Large Exercises}

\begin{enumerate}[\thechapter.\thesection .1]
\item \label{Image is subspace} In the opening of the chapter, we use Theorem \ref{Vector Decomposition}, but we can only do this if $\mathcal P$ is a subspace, meaning that it's closed under vector addition and scalar multiplication.  Prove that any matrix's image is a subspace.

\section{Inverses and pseudo-inverses}
\section{Least Squares}
\section{Graham-Schmidt and QR factorizations}
...
\section{Prove specific form}
\section{Corollaries}
\section{Use to calculate SVD}
\section{Inner products, covariance}
\section{Spectral Theorem}
\section{Taylor series and smooth functions}
...
For any subspace $W$ and any vector $|v\rangle$, $|v\rangle$ can be written uniquely as $|v\rangle=|v_\parallel\rangle+|v_\bot\rangle$, where $|v_\parallel\rangle\in W$ and $\langle w|v_\bot\rangle=0$ for all $w\in W$.
\end{theorem}

...
{\bfseries\Large Algorithm 1: SVD}\label{alg1}
...
\item Given a matrix $A$, let $\left\{A_i\right\}$ be the matrices found in Algorithm \ref{alg1}.  Show that for any $j>i$ an eigenvector corresponding to a non-zero eigenvalue of $A_j$ is also an eigenvector of $A_i$.
